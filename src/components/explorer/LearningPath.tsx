  ...
  return (
    <div className="h-full flex flex-col overflow-hidden">
      ...
      <div className="px-4 pt-4 pb-3 flex-shrink-0">
        ...
        <p className="text-[11px] font-mono" style={{ color: "#6B6B80" }}>
          {repoName ?? "Repository"} · {framework ?? "Unknown"}
        </p>
      </div>
      ...
      >
        ...
        <p className="text-[11px]" style={{ color: "#9090A0" }}>
          {steps.filter((s) => s.status === "completed").length === steps.length
            ...
            : `${steps.length - steps.filter((s) => s.status === "completed").length} of ${steps.length} steps remaining`}
        </p>
      </div>
    </div>
  );
